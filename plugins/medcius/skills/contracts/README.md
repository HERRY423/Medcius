...
**In Claude Code**, install the medcius plugin:

```
/plugin marketplace add HERRY423/Medcius
/plugin install medcius@medcius   # plugin-name @ marketplace-name
```
...
- State (db, observations log) lives at `~/.claude/data/medcius/documents/` — machine-global so learned knowledge carries across projects, persists across plugin upgrades. Override the parent dir with `$CLAUDE_MEDCIUS_DATA` (the server appends `/documents`). The server creates and writes this itself — no sandbox allowlisting needed.
- If a plugin upgrade changes the schema, the server says so on startup and names the file to delete: `~/.claude/data/medcius/documents/data.sqlite` (the `parsed/` cache can stay). The corpus re-ingests automatically on the next `/contracts` — there's no migration.
