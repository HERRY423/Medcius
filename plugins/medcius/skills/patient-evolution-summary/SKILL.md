...
- **产品定位**：一线临床入口是 **HIS 患者页内嵌 / 院内 SSO**。Codex / Trae / WorkBuddy 只用于工程与合成回放，不是病区工作面。本技能不是能够独立诊疗的“临床智能体”。
- **P0 静默试点**：医院落地默认不向医生弹窗、不生成病程草稿、不写回 EHR；只做旁路计算与审计落盘。
- **核心差异化价值**：
...
静默试点不对临床医生开放草稿。工程/建议模式中，医生勾选确认的条目可汇总为【日常查房记录 - 病情演变摘要】文本；草稿末尾附带确认时间戳，由医生在原生 EMR 中核对后手动复制，系统不得自动写回。
