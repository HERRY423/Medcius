...
```bash
cd <this skill dir> && bun install     # or: npm install
```

This pulls [liteparse](https://www.npmjs.com/package/@llamaindex/liteparse) (the `lit` bin, used for PDF/DOCX/XLSX/PPTX, OCR included). RTF decoding is built in with no third-party dependency. Without liteparse, PDFs still work via a `pdftotext -layout` fallback if poppler is installed; other binary formats require liteparse.

## Use

The script uses only Node built-ins (no Bun-specific API), so it runs under either runtime:

```bash
bun  <this skill dir>/scripts/extract.ts <input-file> [--content-type <mime>]
# or, on Node ≥ 23.6 (or ≥ 22.6 with --experimental-strip-types):
node <this skill dir>/scripts/extract.ts <input-file> [--content-type <mime>]
```

Output on stdout:

```json
{ "text": "...", "method": "liteparse | pdftotext | rtf | xml | passthrough", "pages": 12 }
```
