...
A `~/.claude/data/medcius/fraud-detection/out/run-<timestamp>/` directory with:
...
All fetched data + run output land at `~/.claude/data/medcius/fraud-detection/` (override the
parent dir with `$CLAUDE_MEDCIUS_DATA`) — never under the plugin install path.

```bash
cd skills/fraud-detection
export CLAUDE_MEDCIUS_DATA="${CLAUDE_MEDCIUS_DATA:-$HOME/.claude/data/medcius}"
node scripts/fetch-reference.js 2026q3   # one-time: NCCI/PFS/LEIE/MCD/benchmarks → $CLAUDE_MEDCIUS_DATA/fraud-detection/data-cache/ (~2GB)
# follow LOAD-CLAIMS.md to populate $CLAUDE_MEDCIUS_DATA/fraud-detection/data-cache/corpus.duckdb from your claims extract
claude                                   # then: /fraud-detection
```

Watch the per-finding fan-out live with `/workflows`. Or run the deterministic floor alone:
`node scripts/screen.js "$CLAUDE_MEDCIUS_DATA/fraud-detection/data-cache/corpus.duckdb" 2026q3 medicaid` → `$CLAUDE_MEDCIUS_DATA/fraud-detection/out/run-*/referrals.json`.
...
- **Your claims** → [LOAD-CLAIMS.md](LOAD-CLAIMS.md). Map your 837/encounter feed into six tables
  ...
  [`claims-schema.sql`](claims-schema.sql) and land them in `~/.claude/data/medcius/fraud-detection/data-cache/corpus.duckdb`.
