...
- **Render for review.** The skill writes packets to `$CLAUDE_MEDCIUS_DATA/fraud-detection/out/`; the payer's SIU workflow
...
at `~/.claude/data/medcius/fraud-detection/` — override the parent dir with
`$CLAUDE_MEDCIUS_DATA` (each skill appends its own name). Below, `data-cache/` and `out/` are
subdirectories of `$CLAUDE_MEDCIUS_DATA/fraud-detection`. Resolve it once at the start of a run:
```bash
export CLAUDE_MEDCIUS_DATA="${CLAUDE_MEDCIUS_DATA:-$HOME/.claude/data/medcius}"
```
...
1. **Get the payer's claims into `corpus.duckdb`.** Open with: *"Where do your adjudicated claims
   ...
   and the user from "I don't know" to a populated `$CLAUDE_MEDCIUS_DATA/fraud-detection/data-cache/corpus.duckdb`. If they
...
2. **Seed the public reference layer (first run / new quarter only).** Detectors cite against
   `$CLAUDE_MEDCIUS_DATA/fraud-detection/data-cache/reference/<quarter>/reference.duckdb`. If that file is missing for the
...
3. **Create the run directory.** Each invocation lands in its own minute-stamped directory so prior
   ...
   ```bash
   export FRAUD_OUT_DIR="$CLAUDE_MEDCIUS_DATA/fraud-detection/out/run-$(date +%Y%m%d-%H%M)"
...
4. **Run the investigation** by calling the **Workflow** tool with:
   ...
   - `args`: `{ "corpusDb": "<abs path to corpus.duckdb>", "quarter": "2026q3", "lob": "medicaid", "pluginRoot": "${CLAUDE_PLUGIN_ROOT}/skills/fraud-detection", "dataRoot": "<abs $CLAUDE_MEDCIUS_DATA/fraud-detection>", "outDir": "<abs FRAUD_OUT_DIR from step 3>" }`
...
- **Reference data** — `scripts/reference-data.js` loads `$CLAUDE_MEDCIUS_DATA/fraud-detection/data-cache/reference/` (NCCI/MUE, LEIE, PFS,
  enrollment), fetched by `scripts/fetch-reference.js`, versioned by date-of-service quarter.
- **Enrichment** — `scripts/enrichment.js` loads `$CLAUDE_MEDCIUS_DATA/fraud-detection/data-cache/enrichment/`, fetched by `fetch-enrichment.js`.
