...
Write `~/.claude/data/medcius/fraud-detection/ingest/<payer>/mapping.md` (override the parent
`~/.claude/data/medcius` with `$CLAUDE_MEDCIUS_DATA`): for **every** canonical column,
...
Produce `~/.claude/data/medcius/fraud-detection/ingest/<payer>/ingest.sql` (or `.js` for X12/FHIR):
...
Target DB: `~/.claude/data/medcius/fraud-detection/data-cache/corpus.duckdb`. Apply `claims-schema.sql`
...
Run the converter, then `~/.claude/data/medcius/fraud-detection/ingest/<payer>/validate.sql`:
...
Print the absolute path to `~/.claude/data/medcius/fraud-detection/data-cache/corpus.duckdb`. The main skill resumes with
