...
**Delegate, don't read them yourself.** A visual read is ~18 pages of images per contract — done in the conductor it floods the context that still has to compose the answer. Spawn one subagent per doc (all in one message, they run concurrently) — **typed `medcius:documents-reader-cli` (or `medcius:documents-reader-mcp`), never untyped**: an untyped spawn can resolve to an agent the corpus folder defines. Give it the source path and the rubric, have it Read in windows of ≤20 pages (the Read tool's cap; >10-page PDFs require the pages param) until every page is seen, and return compact `FACT | value | p<page>` lines only.
