...
- Medcius 是安装到宿主 Agent 的受约束能力包，不是独立临床平台。一线临床入口是 HIS 患者页内嵌（`/his/embed`）或院内 SSO；Codex/Trae 只用于工程回放。运维对象是：插件包本体、本地 MCP servers、院内 HTTP API/静默 CDS、P3 视图库只读连接器。
